# GitHub Learning Resources

一份按学习目标分类的高质量 GitHub 学习资源导航。

本仓库只整理公开项目链接、学习顺序和使用建议，不复制原项目内容。适合放在个人 GitHub 主页、收藏夹仓库，或继续拆分成多个专题仓库。

## 入选标准

- 资源有明确学习价值，不只是工具清单。
- 项目或社区长期被开发者引用，内容结构清楚。
- 优先选择免费、开源、可自学的材料。
- 中英文资源兼收，中文学习路径单独整理。
- 避免堆数量，保留适合长期学习的核心项目。

## 快速学习路线

### 1. 新手起步

1. [GitHub Skills: Introduction to GitHub](https://github.com/skills/introduction-to-github)
2. [first-contributions](https://github.com/firstcontributions/first-contributions)
3. [free-programming-books](https://github.com/EbookFoundation/free-programming-books)
4. [developer-roadmap](https://github.com/kamranahmedse/developer-roadmap)

### 2. 计算机基础

1. [OSSU Computer Science](https://github.com/ossu/computer-science)
2. [CS-Notes](https://github.com/CyC2018/CS-Notes)
3. [every-programmer-should-know](https://github.com/mtdvio/every-programmer-should-know)
4. [papers-we-love](https://github.com/papers-we-love/papers-we-love)

### 3. 编程和项目实践

1. [freeCodeCamp](https://github.com/freeCodeCamp/freeCodeCamp)
2. [project-based-learning](https://github.com/practical-tutorials/project-based-learning)
3. [build-your-own-x](https://github.com/codecrafters-io/build-your-own-x)
4. [The Algorithms - Python](https://github.com/TheAlgorithms/Python)

### 4. 面试和系统设计

1. [coding-interview-university](https://github.com/jwasham/coding-interview-university)
2. [system-design-primer](https://github.com/donnemartin/system-design-primer)
3. [javascript-algorithms](https://github.com/trekhleb/javascript-algorithms)
4. [labuladong 的算法笔记](https://github.com/labuladong/fucking-algorithm)

### 5. Web、AI、数据方向

1. [Web Dev For Beginners](https://github.com/microsoft/Web-Dev-For-Beginners)
2. [ML For Beginners](https://github.com/microsoft/ML-For-Beginners)
3. [Generative AI For Beginners](https://github.com/microsoft/generative-ai-for-beginners)
4. [AI Agents For Beginners](https://github.com/microsoft/ai-agents-for-beginners)

## 分类目录

- [Git 和 GitHub](categories/01-git-github.md)
- [计算机科学基础](categories/02-computer-science.md)
- [编程语言和项目实践](categories/03-programming-projects.md)
- [算法、数据结构和面试](categories/04-algorithms-interviews.md)
- [Web、后端和工程能力](categories/05-web-backend-engineering.md)
- [AI、机器学习和数据科学](categories/06-ai-ml-data.md)
- [中文优质资源](categories/07-chinese-resources.md)
- [工具、资料库和长期收藏](categories/08-reference-libraries.md)

## 如果你想分类建多个库

见 [REPO_SPLIT_PLAN.md](REPO_SPLIT_PLAN.md)。里面给了推荐仓库名、简介和适合放的分类，之后可以按这个计划拆成多个 GitHub 仓库。

## 维护建议

- 每月检查一次失效链接。
- 新增资源时先写“为什么值得学”，再放链接。
- 不要把同类资源堆太多，宁可保留 5 个真正会用的。
- 不复制课程正文、书籍内容或原项目 README，大段内容请回到原链接阅读。
